...
def wiki_enemies(event : str = "", ps : bool = False, rogue : str = "", sss : str = "", temp : bool = False, show : bool = False) -> dict :
    ...
    if ps:
        if PS_OP:
            ...
            ps_ops = DB["json_handbook_infoEN"]["handbookStageTime"][-1]["charSet"]
        for ps_op in ps_ops:
            ps_stage_data = DB["json_handbook_infoEN"]["handbookStageData"][ps_op]
            stages[ps_stage_data["code"]] = ps_stage_data
    elif rogue:
        ...
        for stage in RO_DB["details"][rogue]["stages"]:
            ...
            printc(stage, re.match(r'ro(\d+)_n_(.+?)', stage) or re.match(r'ro(\d+)_t_(.+?)', stage))
            if re.match(r'ro(\d+)_n_(.+?)', stage) and stage.replace("_n_", "_e_", 1) in RO_DB["details"][rogue]["stages"]:
                stages[stage]["hard_stage"] = stage.replace("_n_", "_e_", 1)
            elif re.match(r'ro(\d+)_t_(.+?)', stage) and stage.replace("_t_", "_e_t_", 1) in RO_DB["details"][rogue]["stages"]:
                stages[stage]["hard_stage"] = stage.replace("_t_", "_e_t_", 1)
    elif sss:
        ssszone : list = DB["json_climb_towerEN"]["towers"][sss]["levels"]
        data["zone"][sss] = ssszone
        for stage in ssszone:
            stages[stage] = DB["json_climb_towerEN"]["levels"][stage]
            data["zone_node"].setdefault(stage, {"prev":[], "next":[]})
            stage_index = ssszone.index(stage)
            if stage_index - 1 in range(len(ssszone)):
                data["zone_node"][ssszone[stage_index - 1]]["next"].append(stage)
                data["zone_node"][stage]["prev"].append(ssszone[stage_index - 1])
    else:
...
def wiki_article(
    ...
    enemy_rune      = [
                        ...
                        "level_enemy_replace", "level_hidden_group_enable", "level_hidden_group_disable", "level_predefine_tokens_random_spawn_on_tile"
                    ]
    non_enemy_rune  = [
    ...
    skip_buffs      = [
                        "act27sisde_enemy_global_buff", 
                        "legion_mode_enemy_boost",
                        "mainline12_sightManager", 
                        "night_map_default", 
                        "rogue_4_parasitic_buff", "rogue_4_lock_all_entities_when_born", 
                        "rogue_5_enemy_fall_down_log", "rogue_5_deify_common_buff", "rogue_5_heaven_inherit_unit_status",
                        "sandbox_inherit_status", 
                        "strife_mode_feature", 
                    ]
    
    norm_env = [
                {'ui_plugin_name': 'common_camera_move_ui_plugin', 'camera_plugin_name': 'common_camera_move_half_second_plugin'},
                {'key': 'env_007_act27side', 'remaining_respawn_time': 1.0, 'damage_scale': 1.5, 'damage_resistance': 0.75, 'ep_damage': 350.0, 'enemy_damage': 0.0, 'ally_damage': 250.0, 'duration': 999.0},
    ...
    def desc_cond_writer(desc_cond : str) -> str:
        ...
        desc = re.sub(r'<@rolv.rem>(.+?)<\/>', r"{{Color|\1|rolv}}", desc)
        # stage mechanic
    ...
    def enemies_lister(def_data : dict) -> dict:
        ...
        for enemy_class in enemy_classes:
            for enemy in sorted(enemy_list[enemy_class].keys(), key = (lambda enemy_key : big_data["enemies"][enemy_key]["handbook"]["sortId"]) if sortby_sortID else (lambda enemy_key : re.sub(r'(\d+)', lambda m : f'{m.group(1).zfill(5)}', big_data["enemies"][enemy_key]["handbook"]["enemyIndex"]))):
    ...
    def tile_writer(def_data : list, static_data : list) -> str:
        ...
        for tile in def_data:
            ...
            match tile_id:
                ...
                case "tile_merope" | "tile_rarope":
                    default_effect = [
                                    ]
                    if (tile["blackboard"] in default_effect and not tile["effects"]) or (not tile["blackboard"] and not tile["effects"]):
                        pass
                    else :
                        printc(f'New {Y}{tile_id}{RE} case just drop : {B}{tile}{RE}') 
                case "tile_sandrope_2":
                    default_effect = [
                                        [{'key': 'direction', 'value': 0.0, 'valueStr': 'left'}],
                                        [{'key': 'direction', 'value': 0.0, 'valueStr': 'down'}],
                                    ]
                    if (tile["blackboard"] in default_effect and not tile["effects"]) or (not tile["blackboard"] and not tile["effects"]):
                        pass
                    else :
                        printc(f'New {Y}tile_sandrope_2{RE} case just drop : {B}{tile}{RE}') 
                case "tile_quicksand":
                    default_effect = [
                                        [{'key': 'move_speed', 'value': 0.7, 'valueStr': None}, {'key': 'interval', 'value': 0.2, 'valueStr': None}, {'key': 'falldown_stack_count', 'value': 75.0, 'valueStr': None}],
                                    ]
                    if (tile["blackboard"] in default_effect and not tile["effects"]) or (not tile["blackboard"] and not tile["effects"]):
                        pass
                    else :
                        printc(f'New {Y}tile_quicksand{RE} case just drop : {B}{tile}{RE}') 
                case "tile_start_cooperate" | "tile_end_cooperate":
                    default_effect = [
                                        [{'key': 'side', 'value': 0.0, 'valueStr': 'Shared'}],
                                        [{'key': 'side', 'value': 0.0, 'valueStr': 'Side_A'}],
                                        [{'key': 'side', 'value': 0.0, 'valueStr': 'Side_B'}],
                                    ]
                    if (tile["blackboard"] in default_effect and not tile["effects"]) or (not tile["blackboard"] and not tile["effects"]):
                        pass
                    else :
                        printc(f'New {Y}{tile_id}{RE} case just drop : {B}{tile}{RE}')
                case "tile_reedf" | "tile_deepwater":
                    default_effect = [
                                    ]
                    if (tile["blackboard"] in default_effect and not tile["effects"]) or (not tile["blackboard"] and not tile["effects"]):
                        pass
                ...
                case "tile_forbidden" | "tile_start" | "tile_end" | "tile_fence_bound" | "tile_hole" | "tile_deepwater":
                    default_effect = [
                                        [{'key': 'side', 'value': 0.0, 'valueStr': 'Shared'}],
                                        [{'key': 'side', 'value': 0.0, 'valueStr': 'Side_A'}],
                                        [{'key': 'side', 'value': 0.0, 'valueStr': 'Side_B'}],
                                        [{'key': 'side', 'value': 0.0, 'valueStr': 'Side_B'}, {'key': 'branch_id', 'value': 0.0, 'valueStr': 'e2'}],
                                    ]
                    if (tile["blackboard"] in default_effect and not tile["effects"]) or (not tile["blackboard"] and not tile["effects"]):
                        pass
                    else:
                ...
                case "tile_dygmny_1" | "tile_dygmny_2" | "tile_dygmny_3":
                    pass
                case "tile_corrosion":
                    default_effect = [
                                        [{'key': 'def', 'value': 0.5, 'valueStr': None}]
                    ]
                    if tile["blackboard"] in default_effect and not tile["effects"]:
                        tile_result.append(f'[[Corrosive Grounds]] halve the DEF of friendly units on them.')
                    else:
                        printr(f'new Terrain to add : {Y}{tile_id}\n\t{G}{tile["blackboard"]}\n\t{B}{tile["effects"]}{RE}')
                case _ :
                    printr(f'new Terrain to add : {Y}{tile_id}\n\t{G}{tile["blackboard"]}\n\t{B}{tile["effects"]}{RE}')
    ...
    def enemy_name_search(key : str | list, enemyDbRefs : dict) -> str:
        def searching(enemy):
        ...
        if isinstance(key, str):
            if key.find("|") != -1:
                return join_and(sorted([searching(enemy) for enemy in key.split("|") if enemy not in ["null", "", " "]], key = lambda x : x.replace("'", "")))
            else:
            ...
            if rune["difficultyMask"] == "ALL" or rune["difficultyMask"] == diff:
                match rune["key"]:
                    case "env_system_new":
                        ...
                        elif rune_key == "env_coop_sail_boat_mgr":
                            boat_total_time     = temp["blackboard"]["boat_total_time"]
                    ...
                    case "map_tile_blackb_assign":
                        ...
                        if rune["blackboard"] in default_effect:
                            ...
                            match rune["blackboard"]["tile"]:
                                case "tile_reed|tile_reedf|tile_reedw":
                                    reed_ignite     = rune["blackboard"]["ignite_duration"]
                                    reed_extinct    = rune["blackboard"]["extinct_duration"]
                                    reed_cooldown   = rune["blackboard"]["cooldown_duration"]
                                    reed_arts       = rune["blackboard"]["damage"]
                                    reed_burn       = rune["blackboard"]["ep_damage"]
                                    rune_writer.append(f'\n[[Reeds|Burning Reeds]] deal {reed_arts:g} Arts damage and {reed_burn:g} {{{{G|Burn Damage}}}} every second, ignites the adjacent Reeds every {reed_ignite:g} seconds, take {reed_extinct:g} seconds to be extinguished and cannot be ignited for {reed_cooldown:g} seconds afterward.')
                                case _:
                                    printr(f'{Y}New {rune["key"]} just drop : {B}{rune["blackboard"]}')
                    case "env_gbuff_new_with_verify":
                        if temp["blackboard"].get("key", "") in ["cooperate_enemy_side_shared", "cooperate_boat_enemy_die"]: # enemy
                            continue
                            ...
                            enemy_name      = [enemy_name_search(enemy, enemyDbRefs) for enemy in attr_enemy.split("|") if enemy != 'null'] if attr_enemy else []
                    ...
                    case "char_skill_cd_mul":
                        ...
                        if temp["blackboard"] and [key for key in temp["blackboard"] if key not in skip_rune]:
                            printr(f'{Y}New {rune["key"]} bb just drop : {B}{rune["blackboard"]}')
                            exit()
                        rune_writer.append(f'\n{char_add_name if char_add_name else "All"} skill cost {"reduced" if 1 - char_skill_cd >= 0 else "increased"} by {abs((1 - char_skill_cd))*100:g}%')
                    case "char_skill_cd_add":
                        char_skill_cd   = temp["blackboard"].pop("value")
                        char_add = temp["blackboard"].pop("char") if "char" in rune["blackboard"] else ""
                        char_add_name = join_and([DB["json_characterEN"][char]["name"] if char in DB["json_characterEN"] else DB["json_character"][char]["appellation"]  for char in char_add.split("|")]) if char_add else ""
                        if temp["blackboard"] and [key for key in temp["blackboard"] if key not in skip_rune]:
                            printr(f'{Y}New {rune["key"]} bb just drop : {B}{rune["blackboard"]}')
                            exit()
                        rune_writer.append(f'\n{char_add_name if char_add_name else "All"} skill cost {"reduced" if char_skill_cd < 0 else "increased"} by {abs(char_skill_cd):g}')
    ...
    def eaddendum_lister(stage_key : str):
        ...
        for enemy_ref in big_data["stage"][stage_key]["enemyDbRefs"]:
                                    ...
                                    for blackboard in enemy_ref["overwrittenData"][key]:
                                        ...
                                        '''if blackboard_list and not dupe_value:
                                            ...
                                            if blackboard["key"] in blackboard_listed:
                                                ...
                                                printr(f'{Y}{stage}{RE} new blackboard key {Y}{blackboard["key"]} ({blackboard["valueStr"] or blackboard["value"]}){RE} for : {Y}{enemy_id}{R}({stage_key}){RE}')'''
                                    if blackboard_list:
                                        ...
                                        if big_data["enemies"][enemy_id]["lv"][enemy_lv][key]:
                                            for skill in big_data["enemies"][enemy_id]["lv"][enemy_lv][key]:
                                                if skill["prefabKey"] != skill_key["prefabKey"]:
                                                    continue
                                                if skill_key and skill == skill_key:
                                                    break
                                                else:
                                                    #printc(skill, skill_key)
                                                    for s_key in skill_key:
                                                        if s_key == "blackboard" and skill_key[s_key] and not (skill[s_key] == skill_key[s_key] == None):
            ...
            if enemy_overwrittenData:
                eaddendum.append((enemy_id, {"data" :enemy_overwrittenData, "id" : enemy_ref["id"]}))
        
        #if eaddendum: printc("eaddendum", stage, eaddendum)
        return eaddendum
    
    def eaddendum_writer(eaddendum, runes = [], buffs = [], enemyDbRefs = {}, diff = "", extra = ""):
        def eaddendum_stat_writer(key : str, value : str|int|dict|list, enemy_id : str = ""):
            def skill_lister(k : str, v : str|int):
                match k:
                    case "initCooldown" :
                    ...
                    case "Skill-BB-heal_scale":
                        return f'heal is increased to {v*100:g}% ATK'
                    case "blackboard":
                        return v
                    case _:
            ...
            match key:
                ...
                case "enemy_exclude":
                    return f'Excluding -> {enemy_name_search(value.split("|"), enemyDbRefs)}'
                case "notCountInTotal":
                    if value:
                        return "not count toward enemy counter"
                case "talentBlackboard":
                    talent_skip = [
                        [{'key': 'sleepwalking.unmove_duration', 'value': 0.0, 'valueStr': None}], [{'key': 'sleepwalking.unmove_duration', 'value': 0.1, 'valueStr': None}],
                        [{'key': 'revive.unmove_duration', 'value': 0.0, 'valueStr': None}]
                    ]
                    ...
                    if len(value) == 1 and value[0]["value"] == 0.0 and value[0]["valueStr"] == None or value in talent_skip:
                        ...
                        match temp:
                            ...
                            case {"invincible.duration" : bb_duration}:
                                return f'(invulnerable) for {bb_duration:g} seconds'
                            case {"StoneSkin_2.atk_scale" : bb_atk}:
                                if enemy_id == "enemy_1523_mandra":
                                    return f'Stoneshield deals Arts damage equal to {bb_atk*100:g}% ATK every second'
                                else:
                                    pass
                            case {"passive.enemy_key" : bb_enemy_key}:
                        ...
                        match temp:
                            case {'yinyang.buff_yinyang[same].atk_scale': same_atk_scale, 'yinyang.buff_yinyang[diff].atk_scale': diff_atk_scale}:
                                if enemy_id in ["enemy_1123_spsbr", "enemy_1127_sptrkg", "enemy_1129_spklr"]:
                                    return f'ATK multiplier against targets with a matching/opposing [[Hui and Ming|Hui/Ming attribute]] is changed to {same_atk_scale*100:g}%/{diff_atk_scale*100:g}%'
                            case _:
                                pass
                    elif len(value) == 3:
                        match temp:
                            ...
                            case {'yinyang.dynamic': dynamic, 'yinyang.buff_yinyang[same].atk_scale': same_atk_scale, 'yinyang.buff_yinyang[diff].atk_scale': diff_atk_scale}:
                                if enemy_id in ["enemy_1123_spsbr", "enemy_1127_sptrkg", "enemy_1129_spklr"]:
                                    return f'ATK multiplier against targets with a matching/opposing [[Hui and Ming|Hui/Ming attribute]] is changed to {same_atk_scale*100:g}%/{diff_atk_scale*100:g}%'
                            case _:
                                pass
        ...
        eaddendum_dict = {
                            ...
                            "baseAttackTime": "BAT", 
                            "enemyTags" : "enemyTags",
                            "notCountInTotal" : "notCountInTotal",
                    ...
                    match rune["key"]:
                            ...
                            for key in rune["blackboard"]:
                                ...
                                if key in eaddendum_skip or value == 0:
                                    ...
                                    if isinstance(value, str):
                                        ...
                                        temp_stat.append(eaddendum_dict[key])
                                    elif (rune["key"] == "enemy_attribute_mul" and value - 1) or (rune["key"] == "enemy_attribute_add" and value):
                                        temp_value.append(f'{value:g}' if rune["key"] == "enemy_attribute_add" else f'{value - 1:.0%}')
                                        temp_stat.append(eaddendum_dict[key])
                                    else :
                                        printr(key, value, rune["blackboard"])
                                        continue
                                elif key == "enemy":
                                    all_enemy = value.split("|")
                                    try:
                                        enemy_name = join_and([enemy_name_search(enemy, enemyDbRefs) for enemy in all_enemy])
                                    except:
                                        enemy_name = join_and([enemy_name_search(enemy, enemyDbRefs) for enemy in all_enemy if enemy in enemyDbRefs])
                        ...
                        case "env_gbuff_new_with_verify":
                            if rune["blackboard"]["key"] in ['attr_common_global_buff[all][trap]']:
                                ...
                                share_enemy = join_and([DB["json_enemy_handbookEN"]["enemyData"][enemy]["name"] if enemy in DB["json_enemy_handbookEN"]["enemyData"] else ENEMY_NAMES_TL.get(enemy, f'{enemy_name_search(enemy, enemyDbRefs)}({enemy})') for enemy in share_enemy_id if not re.match(r'enemy_.+?enemy_.+?', enemy)])
                                eaddendum_result.append(f'\n{wiki_story(share_enemy)} will deduct both players [[Life Point|Life Points]] upon entering a [[Protection Objective]].')
                            elif temp["blackboard"]["key"] == "cooperate_boat_enemy_die":
                                score       = temp["blackboard"]["score"]
                                enemies     = temp["blackboard"]["enemy"]
                                enemy_name  = join_and([enemy_name_search(enemy, enemyDbRefs) for enemy in enemies.split("|") if enemy not in ['null', ""]] if enemies else [])
                                eaddendum_result.append(f'\nGain {score:g} point when defeat {wiki_story(enemy_name)}.')
                            else :
                        ...
                        case "enemy_dynamic_ability_new":
                            known_key = {
                                            ...
                                            "frozenimmue"   : "immune to {{G|Freeze|Freeze}}"
                                        }
                        ...
                        case "enemy_weight_add":
                            scale = temp["blackboard"].pop("value")
                            get_enemy = temp["blackboard"].pop("enemy").split("|") if "enemy" in temp["blackboard"] else ""
                            try:
                                enemy_name = join_and([enemy_name_search(enemy, enemyDbRefs) for enemy in get_enemy])
                            except:
        ...
        for enemy in eaddendum:
            ...
            for k,v in enemy[1]["data"].items():
                ...
                if k not in eaddendum_dict:
                    ...
                    parsing = eaddendum_stat_writer(k, v, enemy_key)
                    if parsing : eaddendum_parse.append(parsing)
            if eaddendum_parse:
                eaddendum_result.append(f'\n*{wiki_story(enemy_name)} have {join_and(eaddendum_parse)}.{f' ({enemy_ref_id})' if enemy_key != enemy_ref_id else ""}')
    ...
    def operators_predefine_writer(def_comp_data, def_preauto_data, def_auto_data, fixed, except_op : str = ""):
        ...
        if def_comp_data:
            for op in def_comp_data:
                if op["inst"]["characterKey"] == except_op: continue
    ...
    def stage_article_data(data : dict, stage : str, mode : str, diff : str = "") -> dict:            
        def drop_lister(def_data : list) -> dict:
            ...
            stage_drop_list   = {"firstdrop": [], "regdrop" : [], "regrate" : [], "specdrop" : [], "specrate" : []}
            for drop_type in drop_types:
                for drop in sorted(drop_list[drop_type].keys(), key = lambda k : -100000000000 if k == "4002" else DB["json_item"]["items"][k]["sortId"] if k in DB["json_item"]["items"] else 100000000000):
                    if drop.startswith("furni"):
                        ...
                        drop_itemType = DB["json_item"]["items"][drop]["itemType"]
                    ...
                    if drop_type == "COMPLETE" and drop_name != "Originite Prime":
                        stage_drop_list["firstdrop"].append(drop_name)
                        if drop_itemType == "ACTIVITY_ITEM": stage_drop_list.setdefault("firstqty", True)
                    elif drop_type == "NORMAL":
                        stage_drop_list["regdrop"].append(drop_name)
                        stage_drop_list["regrate"].append(str(drop_rate))
                        if drop_itemType == "ACTIVITY_ITEM": stage_drop_list.setdefault("regqty", True)
    ...
    def stage_operation_list(stage_key : str, info_dict : dict = {}, data_dict : dict = {}):
        if info_dict and data_dict:
            ...
            return re.sub(r'\n+', "\n", f'''{{{{Operation list cell
                                            {f'|type = {"cinematic" if data_dict["isCINEMATIC"] else "tutorial"}' if data_dict["isCINEMATIC"] or data_dict["isTRAINING"] else ""}
                                            ...
                                            {f'|firstdrop = {", ".join(data_dict["drop_list"]["firstdrop"])}' if data_dict["drop_list"]["firstdrop"] else ""}
                                            {f'|firstqty = ' if data_dict["drop_list"].get("firstqty") else ""}
                                            {f'|regdrop = {", ".join(data_dict["drop_list"]["regdrop"])}' if data_dict["drop_list"]["regdrop"] else ""}
                                            {f'|regqty = ' if data_dict["drop_list"].get("regqty") else ""}
    ...
    def ig_article_data(data : dict, stage_key : str):
        ...
        def ig_group_diff(isEN : bool):
            ...
            EVENT_DATA = DB["json_activityEN"]["activity"]["MULTIPLAY_V3"][event_code] if isEN else DB["json_activity"]["activity"]["MULTIPLAY_V3"][event_code]
            ...
            group_1 = EVENT_DATA["mapModeDataDict"][ig_stage_mode]["name"] if isEN else IG_MODE.get(ig_stage_mode, EVENT_DATA["mapModeDataDict"][ig_stage_mode]["name"])
            group_2 = EVENT_DATA["mapDiffDataDict"][ig_stage_diff]["name"] if isEN else IG_DIFF.get(ig_stage_diff, EVENT_DATA["mapDiffDataDict"][ig_stage_diff]["name"])
        ...
        isEN = event_code in DB["json_activityEN"]["activity"]["MULTIPLAY_V3"]
        ig_group, ig_diff = ig_group_diff(isEN)
    ...
    def ig_article_writer(ig_data : dict):
        if ig_data["isEN"]:
            ...
            targetMissionDataDict = DB["json_activity"]["activity"]["MULTIPLAY_V3"][event_code]["targetMissionDataDict"]
            writer_header = f'''{{{{Spoiler notice|article}}}}\n{{{{Translation|article}}}}\n'''
        return writer_header + f'''{{{{IG operation info
                ...
                |elite = {ig_data["elite"]}
                |boss = {ig_data["boss"]}
                |eaddendum = {eaddendum_writer(ig_data["eaddendum"], ig_data["rune"], ig_data["globalBuffs"], ig_data["enemyDbRefs"], ig_data["ig_diff"])}
        ...
        match mode :
            ...
            case "info":
                ...
                return {
                            ...
                            "desc"      : desc_cond_writer(data["stage_data"][stage]["description"]),
                            "note"      : "",
                    }
            ...
            case "data":
                ...
                return {
                            "stage_id"      : stage,
                            "cond"          : desc_cond_writer(data["stage_data"][stage].get("eliteDesc", "")),
                            "unit_limit"    : global_deploy(data["stage"][stage]["runes"], data["stage"][stage]["options"]["characterLimit"], diff_type),
                            "enemies"       : sum(data["enemies_stage"][stage]["counter"][0:2]),
                            "dp"            : global_dp(data["stage"][stage]["runes"], data["stage"][stage]["options"]["initialCost"], diff_type),
                            "dp_regen"      : data["stage"][stage]["options"]["costIncreaseTime"],
        ...
        match mode:
            ...
            case "data":
                return f'''{{{{IS operation data
                                ...
                                |elite = {is_data["elite"]}
                                |boss = {is_data["boss"]}
                                |eaddendum = {eaddendum_writer(is_data["eaddendum"], is_data["rune"], is_data["globalBuffs"], is_data["enemyDbRefs"], is_data["diff_type"])}
                                }}}}'''.replace("                                ", "")

    def ps_article_data(data : dict, stage_key : str):
        # https://arknights.wiki.gg/wiki/Template:PS_info
        enemies_data = enemies_lister(data["enemies_stage"][stage_key])
        return {
                    "stage_key"     : stage_key,
                    "operator"      : data["stage_data"][stage_key]["charId"],
                    "title"         : data["stage_data"][stage_key]["name"],
                    "desc"          : data["stage_data"][stage_key]["description"],
                    "promotion"     : data["stage_data"][stage_key]["unlockParam"][0]["unlockParam1"],
                    "level"         : data["stage_data"][stage_key]["unlockParam"][0]["unlockParam2"],
    ...
    def ps_article_writer(ps_data : dict):
        return f'''{{{{PS info
                    |operator = {CHARACTER_DATA.getname(ps_data["operator"])}
                    |title = {ps_data["title"]}
                    |desc = {ps_data["desc"]}
                    |promotion = {ps_data["promotion"]}
                    |level = {ps_data["level"]}
                    |unit limit = {ps_data["unit limit"]}
                    |dp = {ps_data["dp"]}
                    |deployable = {ps_data["deployable"]}
                    |static = {ps_data["static"]}
                    |terrain = {tile_writer(ps_data["terrain"], (big_data["stage"][ps_data["stage_key"].split("#")[0]]["predefines"] or {}).get("tokenInsts", []))}
                    |addendum = {addendum_writer(ps_data["rune"], ps_data["globalBuffs"], maxPlayTime = ps_data["maxPlayTime"], DP = ps_data["dp_regen"], configBlackBoard = ps_data["configBlackBoard"], enemyDbRefs = ps_data["enemyDbRefs"])}
                    |enemies = {ps_data["enemies"]}
                    |normal = {ps_data["normal"]}
                    |elite = {ps_data["elite"]}
                    |boss = {ps_data["boss"]}
                    |eaddendum = {eaddendum_writer(ps_data["eaddendum"], ps_data["rune"], ps_data["globalBuffs"], ps_data["enemyDbRefs"])}
                    {operators_predefine_writer(ps_data["comp"], ps_data["pre_auto"], ps_data["auto"], ps_data["fixed"], except_op = ps_data["operator"])}}}}}'''.replace("                    ", "").replace("\n\n", "\n")

    def sss_article_data(data : dict, stage_key : str):
        # https://arknights.wiki.gg/wiki/Template:S3_operation_info
        enemies_data = enemies_lister(data["enemies_stage"][stage_key])
        return {
                    "stage_key"     : stage_key,
                    "code"          : data["stage_data"][stage_key]["code"],
                    "name"          : data["stage_data"][stage_key]["name"],
                    "title"         : data["stage_data"][stage_key]["name"] if wiki_trim(data["stage_data"][stage_key]["name"], replace_all=False) != data["stage_data"][stage_key]["name"] else "",
                    "mission"       : event_name,
                    "prev"          : [data["stage_data"][prev_stage]["name"] for prev_stage in data["zone_node"][stage]["prev"]],
                    "next"          : [data["stage_data"][next_stage]["name"] for next_stage in data["zone_node"][stage]["next"]],
                    "danger"        : [bb["value"] for bb in [globalBuffs for globalBuffs in data["stage"][stage_key]["globalBuffs"] if globalBuffs["prefabKey"] == "legion_mode_enemy_boost"][0]["blackboard"] if bb["key"] == "init_stack_cnt"],
                    "desc"          : data["stage_data"][stage_key]["desc"],
                    "unit limit"    : global_deploy(data["stage"][stage_key]["runes"], data["stage"][stage_key]["options"]["characterLimit"]), 
                    "enemies"       : sum(data["enemies_stage"][stage_key]["counter"][0:2]),
                    "lp"            : global_lifepoint(data["stage"][stage_key]["runes"], data["stage"][stage_key]["options"]["maxLifePoint"]),
                    "dp"            : data["stage"][stage_key]["options"]["initialCost"], 
    ...
    def sss_article_writer(sss_data: dict):
        return f'''{{{{S3 operation info
                    |code = {sss_data["code"]}
                    {f'|title = {sss_data["title"]}' if sss_data["title"] else ""}
                    |mission = {sss_data["mission"]}
                    |prev = {", ".join(sss_data["prev"])}
                    |next = {", ".join(sss_data["next"])}
                    |danger = {sss_data["danger"][0]:g}
                    |desc = {desc_cond_writer(sss_data["desc"])}
                    |unit limit = {sss_data["unit limit"]}
                    |dp = {sss_data["dp"]}
                    |lp = {sss_data["lp"]}
                    |enemies = {sss_data["enemies"]}
                    |deployable = {sss_data["deployable"]}
                    |static = {sss_data["static"]}
                    |terrain = {tile_writer(sss_data["terrain"], (big_data["stage"][sss_data["stage_key"].split("#")[0]]["predefines"] or {}).get("tokenInsts", []))}
                    |addendum = {addendum_writer(sss_data["rune"], sss_data["globalBuffs"], DP = sss_data["dp_regen"])}
                    |normal = {sss_data["normal"]}
                    |elite = {sss_data["elite"]}
                    |boss = {sss_data["boss"]}
                    |eaddendum = {eaddendum_writer(sss_data["eaddendum"], sss_data["rune"], sss_data["globalBuffs"], sss_data["enemyDbRefs"])
                    }}}}}'''.replace("                    ", "").replace("\n\n","\n").replace(" ", " ")
    ...
    def vb_article_writer(vb_data, mode):
        ...
        return f'''{{{{construction}}}}
                    ...
                    |elite = {vb_data["elite"]}
                    |boss = {vb_data["boss"]}
                    |eaddendum = {eaddendum_writer(vb_data["eaddendum"], vb_data["rune"], vb_data["globalBuffs"], vb_data["enemyDbRefs"])}
                    }}}}
                    '''.replace("                    ","").replace("\n\n","\n").replace(" ", " ")
        ...
        if not event_code.startswith(("tower_tr_", "tower_n_")) and event_name:
            for tower in DB["json_climb_towerEN"]["towers"]:
                if DB["json_climb_towerEN"]["towers"][tower]["name"] == event_name:
                    event_code = tower
                    break
        elif event_code.startswith(("tower_tr_", "tower_n_")) and not event_name:
            event_name = DB["json_climb_towerEN"]["towers"][event_code]["name"]
    ...
    elif event_type in ["ps"]:
        mode_info = event_type
        page_footer = f'Y{year} Paradox Simulations'
    elif event_type in ["sss"]:
        mode_info = event_type
    ...
    for stage in big_data["stage"].keys():
        if mode_info == "sidestory":
            ...
            if re.match(r'ro(\d+)_e_(\d+|t)_\d+(|_a-z)', stage):
                continue
            ...
            is_track.append(f'{stage_info["stage_id"]} | {stage_info["name"]}')
            if hard_id:
                stage_data_hard = is_article_data(big_data, hard_id, "data")
                stage_article = [f'# {stage}/{hard_id} | {stage_info["name"]}', is_article_writer(stage_info, "info"), "<tabber>", "Normal=", is_article_writer(stage_data, "data"), "|-|Emergency=", is_article_writer(stage_data_hard, "data", True), "</tabber>", f'{{{{{page_footer}}}}}\n']
            else:
                stage_article = [f'# {stage} | {stage_info["name"]}', is_article_writer(stage_info, "info"), is_article_writer(stage_data, "data"), f'{{{{{page_footer}}}}}\n']
            article_data += stage_article
        
        elif mode_info == "ps":
            stage_info = ps_article_data(big_data, stage)
            stage_article = [f'### {stage} | {stage_info["operator"]} : {stage_info["title"]}', ps_article_writer(stage_info), f'{{{{{page_footer}}}}}\n']
            article_data += stage_article
        
        elif mode_info == "sss":
            stage_info = sss_article_data(big_data, stage)
            stage_article = [f'### {stage} | {stage_info["name"]}', f'{{{{Construction}}}}', sss_article_writer(stage_info)]
            article_data += stage_article
