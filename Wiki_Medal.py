...
from pyFunction_Wiki import replace_apos_between
    ...
    for medal in get_data["medalList"]:
        if medal["medalId"].find(activity) != -1:
            if medal.get("originMedal", ""):
                medal_dict[medal["slotId"]].update({"trim" : wikitrim_method(medal.get("getMethod", ""))})
            else :
                medal_dict[medal["slotId"]] = {
                                                "id"            : medal["medalId"],
                                                "name"          : medal["medalName"],
                                                "rarity"        : medal["rarity"],
                                                "getMethod"     : wikitrim_method(medal.get("getMethod", "")),
