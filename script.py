...
from pyFunction_Wiki import load_json, range_template, replace_apos_between, wiki_stage, wiki_story, wiki_trim
...
def enemy_name():
    ...
    for lang in DB_json.__dict__["_member_names_"]:
        for enemy in DB_json[lang].value["enemies"]:
            enemy_key = enemy["Key"]
            enemy_name_dict.setdefault(enemy_key, {})
    ...
    for enemy in enemy_name_dict:
        enemy_name_dict[enemy]["WIKI"] = ""
        for lang in DB_json.__dict__["_member_names_"]:
            if lang not in enemy_name_dict[enemy]:
                continue
            elif lang == "EN":
                enemy_name_dict[enemy]["WIKI"] += f'|name = {enemy_name_dict[enemy][lang].strip()}'
            elif enemy_name_dict[enemy]["WIKI"]:
                enemy_name_dict[enemy]["WIKI"] += f'\n|{lang.lower()}name = {enemy_name_dict[enemy][lang].strip()}'
            else:
                enemy_name_dict[enemy]["WIKI"] += f'|{lang.lower()}name = {enemy_name_dict[enemy][lang].strip()}'
